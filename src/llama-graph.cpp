...
static thread_local int32_t g_mtp_speculative_step = 0;

void llm_graph_set_mtp_speculative_step(int32_t step) {
    GGML_ASSERT(step >= 0);
    g_mtp_speculative_step = step;
}

int32_t llm_graph_get_mtp_speculative_step() {
    return g_mtp_speculative_step;
}

void llm_graph_input_mtp_speculative_step::set_input(const llama_ubatch *) {
}

bool llm_graph_input_mtp_speculative_step::can_reuse(const llm_graph_params &) {
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    if (k->ne[3] > 1 && (uint32_t) k->ne[3] != ubatch.n_seqs_unq) {
        ...
        GGML_ASSERT((uint32_t)(max_s - min_s + 1) == ubatch.n_seqs_unq &&
                "MTP src-kv attn requires active draft seq_ids to be contiguous");
        GGML_ASSERT((int64_t) max_s < k->ne[3] && "MTP assistant seq_id beyond target stream count");
