...
// ggml_escha_mul_mat
//
// Fused decode + matmul for a single Escha ESCHAM projection -- the dense sibling
// of ggml_trellis_mm_id. Same codec and same Hadamard rotations, but there is no
// routing: one weight matrix, every row of x goes through it, so there are no ids
// and no per-expert stride.
//
//   code : [16*K, OC/16, IC/16] i16, K = 2 or 3
//   rin  : [IC]                  f16
//   rout : [OC]                  f16
//   lut  : [65536]              f16 (accepted but unused -- codebook is computed)
//   dep  : [16, 256]             i16
