...
//
// dense sibling of the escha MoE decode: one weight matrix, no routing, so a row
// is just a token. the 2/3-bit codebook is computed from the payload (QTIP trick),
// the Hadamard rotations are applied per 128-block on the activations and the
// accumulator, and the per-channel scales are folded in at the edges.
